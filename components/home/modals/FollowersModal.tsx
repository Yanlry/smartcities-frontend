...
import { UserFollower } from '../../../types/entities/user.types';
...
/**
 * FollowersModal - Premium modal component displaying user followers
 * 
 * Features:
...
const FollowersModal: React.FC<FollowersModalProps> = memo(({
  ...
  const animValues = useRef<AnimationValues>({
    ...
    listItemAnims: followers.map(() => new Animated.Value(0)),
  }).current;

  // Reset animation values when follower list changes
  useEffect(() => {
    animValues.listItemAnims = followers.map(() => new Animated.Value(0));
  }, [followers]);
   ...
   */
  const renderItem = useCallback(({ item, index }: { item: UserFollower; index: number }) => {
   ...
   */
  const keyExtractor = useCallback((item: UserFollower) => item.id.toString(), []);
